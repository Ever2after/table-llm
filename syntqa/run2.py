...
def main():
    ...
    if training_args.do_predict:
        if data_args.predict_split=='train':
            ...
            if data_args.max_predict_samples is not None:
                max_predict_samples = min(len(predict_dataset), data_args.max_predict_samples)
                predict_dataset = predict_dataset.select(range(1000, max_predict_samples))
    ...
    if training_args.do_predict:
        ...
        for i in tqdm(range(0, max_len, b)):
            ...
            input_ids_ = predict_dataset['input_ids'][i:end]
            input_ids = [item + [tokenizer.pad_token_id]*(data_args.max_source_length-len(item)) for item in input_ids_]
            ...
            # input_id 길이 만큼의 앞 토큰 생략 (qwen-2.5-coder-14b-instruct)
            tmp = [item[len(input_ids_[i]):] for i, item in enumerate(tmp)]

            # 빈 토큰을 pad_token_id로 채워주기
