...
def preprocess_function(examples,
    ...
    for i in range(len(examples['statement'])):
        # (1) 문제, 정답, 테이블 정보 가져오기
        question = "Statement: " + examples['statement'][i] + "\n"
        answer = 'yes' if examples['label'][i] == 1 else 'no'
        
        table_content = {
            "header": examples['table'][i]['header'],
            "rows": examples['table'][i]['rows'],
        }
        ...
        if custom:
            ...
            input_source = TABLE_PROCESSOR.process_input(
                ...
                question, 
                []
            ).strip().lower() + '''\nLet's think step by step, then write an SQL query that evaluates the given statement. If the statement is true, return 'yes'; otherwise, return 'no'. Ensure the final answer format is only "Final Answer: {sql query}" form, no other form. The table name is 'my_table'.''' # E.g. "Final Answer: select col1 from my_table where col2 = 'value'".'''
...
if __name__ == '__main__':
    ...
    dataset_dict = load_dataset("./task/tabfact.py")
